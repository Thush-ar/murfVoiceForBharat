...
class Assistant(Agent):
    def __init__(self) -> None:
        ...
        # Day 8 analytics:
        # A call is successful when Palo successfully
        # fetches a practice exercise.
    ...
    async def get_next_exercise(
        ...
        try:
            ...
            # The exercise was successfully fetched.
...
async def my_agent(ctx: JobContext):
    ...
    # Create the Assistant instance explicitly so we can inspect
    # its Day 8 analytics state when the call closes.
    assistant = Assistant()

    # Check room name and remote participants for SIP indicators.
    # This keeps the existing Day 6 SIP detection unchanged.
    ...
    # Make sure this particular call is recorded only once.
    call_recorded = False
    ...
    async def record_call_outcome():
        ...
        outcome = (
            "SUCCESS"
            if assistant.exercise_requested
            else "FAILED"
        )

        logger.info(
            "Day 8 call outcome: channel=%s outcome=%s",
            channel,
            outcome,
        )
        ...
        try:
            ...
            logger.info(
                "Day 8 analytics saved: call_id=%s outcome=%s",
                call_id,
                outcome,
            )
    ...
    def on_session_close(*args):
        logger.info(
            "Agent session closed. Recording Day 8 outcome."
        )

        # EventEmitter callbacks must be synchronous.
        # Start the async database write without blocking the callback.
        asyncio.create_task(record_call_outcome())
